...
const ProductPage: React.FC = () => {
    ...
    const loadProducts = async () => {
        try {
            setLoading(true);
            const { products, totalCount } = await getProducts(page, limit, categoryId, brandId);
    ...
    const loadCategories = async () => {
        ...
        try {
            const { categories } = await getCategories(categoriesPage, limit);
            if (Array.isArray(categories)) {
                setCategories((prev) => {
                    const unique = [
                        ...prev,
                        ...categories.filter((item) => !prev.some((cat) => cat.id === item.id))
                    ];
                    return unique;
                });
